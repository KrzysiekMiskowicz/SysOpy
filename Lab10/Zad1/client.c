...
void init_connection_with_server(char *type, char *destination)
{
...
void listen_for_server_msg()
{
    ...
    while (1)
    {
        ...
        else if (strcmp(command, "exit_") == 0)
        {
            state = END;
            exit(0);
        }
...
int main(int argc, char *argv[])
{
    ...
    signal(SIGINT, exit_);
    init_connection_with_server(type, destination);
    char buffer[MAX_MESSAGE_LENGTH + 1];
